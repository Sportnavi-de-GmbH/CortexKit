// User-feedback invariants, offline — for the ORCHESTRATOR (service 3).
//
// Third copy of this suite (siblings: kb-agent-langsmith-starter/tests/feedback.test.ts,
// SportnaviPartnerRecomandationBot/.../tests/feedback.test.ts). Same live-instance
// traps apply verbatim (score updates MERGE, `.nullish()` is load-bearing, etc).
// What's unique here: every `buildScorePayload`/`buildReasonPayload`/`submitFeedback`
// call takes a THIRD `context: FeedbackContext` argument (`route`, `partnerSessionId`)
// because this service is a router, not a single-purpose answerer — and there is a
// dedicated `describe("FEEDBACK_SOURCE")` block asserting every score this module
// writes is tagged `source: "API"`, the exact distinction this build was required
// to preserve against Langfuse's separate `source: "ANNOTATION"` sidebar writes.
...
const NO_CONTEXT: FeedbackContext = {};
...
describe("score shape", () => {
  ...
  it("targets a trace when known and a session when not — never nothing", () => {
    const onTrace = buildScorePayload(
      { sessionId: "s", turnId: "t", thumb: "up" },
      TRACE,
      NO_CONTEXT,
      env(CREDS),
...
// ---------------------------------------------------------------------------
// FEEDBACK_SOURCE — the exact distinction this build was required to preserve:
// every score this module writes is `source: "API"` (a real visitor vote),
// never conflated with Langfuse's own `source: "ANNOTATION"` sidebar writes.
// ---------------------------------------------------------------------------
describe("FEEDBACK_SOURCE", () => {
  it("is the literal 'API', not a reviewer-facing value", () => {
    expect(FEEDBACK_SOURCE).toBe("API");
  });

  it("tags the thumb score as source: API", () => {
    const s = buildScorePayload(
      { sessionId: "s", turnId: "t", thumb: "up" },
      TRACE,
      NO_CONTEXT,
      env(CREDS),
  ...
  it("tags the reason score as source: API", () => {
...
// ---------------------------------------------------------------------------
// Orchestration context — unique to this service. `route`/`partnerSessionId`
// are resolved SERVER-SIDE (never client-supplied) and attached so a reviewer
// can tell which capability actually produced the rated answer.
// ---------------------------------------------------------------------------
describe("orchestration context", () => {
  it("records which route answered, in the thumb score's metadata", () => {
    const s = buildScorePayload(
      { sessionId: "s", turnId: "t", thumb: "down" },
      TRACE,
      { route: "faq" },
    ...
    expect(s.metadata.route).toBe("faq");
  });

  it("records the route in the reason score too", () => {
    const r = buildReasonPayload(
    ...
    expect(r?.metadata.route).toBe("find_partners");
  });

  it("defaults route to null when unresolved — never omitted, never a guess", () => {
    const s = buildScorePayload(
      { sessionId: "s", turnId: "t", thumb: "up" },
      TRACE,
      NO_CONTEXT,
    ...
    expect(s.metadata.route).toBeNull();
  });

  it("cross-links the partner session id when the route delegated to service 2", () => {
    const s = buildScorePayload(
      { sessionId: "s", turnId: "t", thumb: "down" },
      TRACE,
      { route: "find_partners", partnerSessionId: "partner-sess-9" },
      env(CREDS),
    );
    expect(s.metadata.partnerSessionId).toBe("partner-sess-9");
  });

  it("omits partnerSessionId entirely when the route never delegated to the partner agent", () => {
    const s = buildScorePayload(
      { sessionId: "s", turnId: "t", thumb: "up" },
      TRACE,
      { route: "faq" },
    ...
    expect(s.metadata).not.toHaveProperty("partnerSessionId");
  });
});
...
describe("submit", () => {
  ...
  it("retracts a previous reason on 👍", async () => {
    ...
    await submitFeedback({ sessionId: "s", turnId: "t", thumb: "up" }, TRACE, NO_CONTEXT, {
      fetchImpl,
      env: env(CREDS),
    });
  ...
  it("reports failure instead of throwing when Langfuse rejects the write", async () => {
    ...
    const res = await submitFeedback({ sessionId: "s", turnId: "t", thumb: "up" }, TRACE, NO_CONTEXT, {
      fetchImpl,
      env: env(CREDS),
    });
  ...
  it("never throws, even when the network does", async () => {
    ...
    await expect(
      submitFeedback({ sessionId: "s", turnId: "t", thumb: "up" }, TRACE, NO_CONTEXT, {
        fetchImpl,
        env: env(CREDS),
      }),
...
describe("annotation queue", () => {
  ...
  beforeAll(() => {
    rmSync(".data/langfuse-annotation-queue-markers", { recursive: true, force: true });
  });
...
// ---------------------------------------------------------------------------
// Wire contract. No `surface` field on this service's schema — see
// lib/feedback.ts's header for why the client cannot supply that here.
// ---------------------------------------------------------------------------
