...
export interface ExtractCityDeps {
  ...
  /** Convex backend; defaults to the shared singleton. */
...
export async function resolveCityFuzzy(
  ...
  try {
    best = await convex.resolveCityFuzzy(place, { signal: opts?.signal });
  } catch (err) {
    throw new Error(
...
export async function extractCityAndIntent(
  ...
  const convex = deps.convex ?? getConvex();
  const resolved = await resolveCityFuzzy(cityInput, convex);
  if (!resolved) {
