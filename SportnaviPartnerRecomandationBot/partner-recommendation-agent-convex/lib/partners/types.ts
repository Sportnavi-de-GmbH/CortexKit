...
export interface PartnerLite {
  ...
  distanceKm?: number;
  /**
   * Intent-relevance score (client-side cosine, rounded to 4 dp — see
   * lib/partners/score-relevance.ts). Present for home AND nearby partners
   * when scoring succeeded; absent means "unscored" (embedding unavailable),
   * which downstream code must treat as unknown — never as 0. R13 §4.
   */
  relevance?: number;
}
...
export interface ResolutionMeta {
  ...
  /**
   * How many HOME partners qualified for this specific request (tag-overlap
   * count when tags were supplied, else the relative relevance cutoff — R13
   * §4.4). Optional: absent on fixtures/legacy sets; treat absent as
   * "unknown" and fall back to home.length.
   */
  homeQualified?: number;
  /** Whether home relevance scoring succeeded (false = unscored fallback). */
  homeScored?: boolean;
  warnings: string[];
