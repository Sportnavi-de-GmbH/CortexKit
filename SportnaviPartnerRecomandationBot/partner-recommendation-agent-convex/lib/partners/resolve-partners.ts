 ...
 *
 * CONVEX PORT: this file contains ZERO backend code — it orchestrates the
 * leaf functions, which is exactly why it is unchanged apart from swapping
 * the injected client type to `ConvexBackend`. Every timeout, threshold,
 * dedupe rule and the whole concurrent gap-fill fan-out are identical to the
 * Supabase build, on purpose.
 */

import { performance } from "node:perf_hooks";
import type { ConvexBackend } from "../convex";
import {
  mergeConfig,
...
import { getPartnerEmbeddings } from "./get-partner-embeddings";
import { relevanceCutoffCount, scoreRelevance } from "./score-relevance";
...
export interface ResolvePartnersDeps {
   ...
   *  per search and shared across home relevance scoring AND the concurrent
   *  gap-fill fan-out. Injectable so the unit suite stays fully mocked and
   *  never touches the network. */
  embedTextFn?: typeof embedText;
  /** Injectable stored-embedding fetch for relevance scoring (R13 §4.1) —
   *  the per-build data-layer leaf. Defaults to get-partner-embeddings.ts. */
  getPartnerEmbeddingsFn?: typeof getPartnerEmbeddings;
}
...
export async function resolvePartners(
  ...
  const convex = deps.convex; // may be undefined; leaf fns fall back to getConvex()
  ...
  const getPartnerEmbeddingsFn = deps.getPartnerEmbeddingsFn ?? getPartnerEmbeddings;
  const rankHomeBySimilarity: RankHomeBySimilarityFn =
    deps.rankHomeBySimilarity ??
    ((home, intent, homeCity) =>
      defaultRankHomeBySimilarity(home, intent, homeCity, similaritySearchPartnersFn, convex));
  ...
  // ── Step 2.4: embed the intent ONCE, hoisted (R13 §4.1) ──────────────────
  // Home relevance scoring AND the gap-fill fan-out share this embedding.
  // `null` means "attempted and failed" — downstream degrades honestly.
  let queryEmbedding: number[] | null = null;
  let embeddingWarning: string | undefined;
  await timeStage("intentEmbed", async () => {
    try {
      queryEmbedding = await embedTextFn(intent.text, {
  ...
  // ── Step 2.5: score home partners by intent relevance (R13 §4) ───────────
  // The home city is still fetched WHOLE and nothing is discarded here —
  // scoring only annotates. It is an enhancement layer: any failure degrades
  // to unscored (historical behavior), never fails the search.
  let homeScored = false;
  if (queryEmbedding !== null && home.length > 0) {
    try {
      await timeStage("homeRelevance", async () => {
        const embeddings = await getPartnerEmbeddingsFn(
          home.map((p) => p.id),
          convex,
          timeoutSignal(HOME_FETCH_TIMEOUT_MS),
        );
        const scores = scoreRelevance(queryEmbedding as number[], embeddings);
        for (const p of home) {
          const s = scores.get(p.id);
          if (s !== undefined) p.relevance = s;
        }
        homeScored = scores.size > 0;
      });
    } catch (err) {
      warnings.push(
        `relevance scoring unavailable (${err instanceof Error ? err.message : String(err)}); results not intent-ranked`,
      );
    }
  }

  // How many home partners QUALIFY for this specific request (R13 §4.4).
  // Tags given → a hard, verifiable tag-overlap count (backs the categorical
  // disclosure). No tags → the relative relevance cutoff. Unscored → the
  // historical raw count, so an embedding outage never changes gap behavior
  // for the worse.
  const intentTagSlugs = intent.tags.map((t) => t.trim().toLowerCase()).filter(Boolean);
  const hasTagOverlap = (p: PartnerLite): boolean =>
    p.tags.some((t) => intentTagSlugs.includes(t.trim().toLowerCase()));
  const homeQualified =
    intentTagSlugs.length > 0
      ? home.filter(hasTagOverlap).length
      : homeScored
        ? relevanceCutoffCount(
            home.map((p) => p.relevance).filter((s): s is number => s !== undefined),
            {
              relevanceBand: cfg.relevanceBand,
              minKneeGap: cfg.minKneeGap,
              shortlistMin: cfg.shortlistMin,
              shortlistMax: cfg.maxPartners,
            },
          )
        : home.length;

  // ── Step 3: home already enough (in QUALIFIED partners)? ─────────────────
  if (homeQualified >= cfg.minPartners) {
  ...
  // The gap is counted in QUALIFIED partners (R13 §4.4): a 56-partner city
  // with 3 relevant partners borrows relevant ones from nearby.
  let gap = cfg.minPartners - homeQualified;
  ...
  try {
    neighbors = await timeStage("nearbyCities", () =>
      findNearbyCitiesFn(
        ...
        { convex, cache: deps.cache as never, signal: timeoutSignal(NEARBY_CITIES_TIMEOUT_MS) },
      ),
    );
  } catch (err) {
    ...
    return buildResult(city, home, [], [city.canonical], cfg, warnings, timingsMs, {
      minMet: homeQualified >= cfg.minPartners,
      cappedAtMax: false,
  ...
  await timeStage("gapFill", async () => {
    ...
    // The intent embedding was computed ONCE at Step 2.4 (hoisted so home
    // relevance scoring shares it); `queryEmbedding === null` means the embed
    // failed and each city search degrades to text-only, carrying
    // `embeddingWarning` through to the laundered warnings.
  ...
  // Score the borrowed partners with the SAME relevance function so every
  // shown partner carries a score from one scale (R13 §4.1). Enhancement
  // layer: failure keeps match_partners' raw similarity as the fallback.
  if (queryEmbedding !== null && filled.length > 0) {
    try {
      await timeStage("nearbyRelevance", async () => {
        const embeddings = await getPartnerEmbeddingsFn(
          filled.map((p) => p.id),
          convex,
          timeoutSignal(PER_CITY_SEARCH_TIMEOUT_MS),
        );
        const scores = scoreRelevance(queryEmbedding as number[], embeddings);
      ...
      // Fallback: PartnerLite.similarity (already set) orders nearby partners.
    }
  }

  // Coverage is measured in QUALIFIED partners: relevant home + accepted
  // borrows (which were fetched by intent and floor-checked).
  const minMet = homeQualified + filled.length >= cfg.minPartners;
  const citiesExhausted = !minMet; // ran out of cities/candidates before min (E13)
  if (!minMet)
    warnings.push(
      `Only ${homeQualified + filled.length} matching partner(s) found near "${city.canonical}" (wanted ${cfg.minPartners}).`,
    );
...
async function defaultRankHomeBySimilarity(
  ...
  homeCity: string,
  similaritySearchPartnersFn: typeof similaritySearchPartners,
  convex: ConvexBackend | undefined,
): Promise<Map<number, number>> {
...
function buildResult(
  ...
  const meta: ResolutionMeta = {
    ...
    homeQualified: flags.homeQualified,
    homeScored: flags.homeScored,
    warnings,
    timingsMs,
  };
