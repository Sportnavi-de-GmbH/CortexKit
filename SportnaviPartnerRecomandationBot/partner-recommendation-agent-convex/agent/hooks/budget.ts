...
export function handlersFor(): Record<string, HookHandler> {
  return {
    ...
    // R13 §3.2 (plans/R03): turn boundaries reset only the PER-TURN window.
    // Conversation-scoped accounting (rendered tokens, known names, ledger)
    // must survive turns — the context those tokens occupy survives turns.
    ...
    // Only a dead session drops the conversation-scoped state.
    "session.failed": guard(async (_event, ctx) => {
      clearBudget(ctx.session.id);
    }),

    // eve deleted the tool-role messages: reset the rendered counter, bump
    // the epoch so previously shown partners re-render in full when next
    // needed (R13 §5 / plans/R05).
    "compaction.completed": guard(async (_event, ctx) => {
      onCompactionCompleted(ctx.session.id);
    }),
