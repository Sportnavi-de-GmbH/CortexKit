...
describe("buildRecommendations — default hydration timeout wiring", () => {
  it("applies a supplied AbortSignal to the default getPartnerProfiles call", async () => {
    const home = [lite(1)];
    const set = setOf(home, [], ["Bochum"]);
    ...
    await buildRecommendations(
      { set, finalRecommendations: 1 },
      { convex, signal: controller.signal },
    );
...
describe("buildRecommendations — hydration and disclosure integrity", () => {
  ...
  it("never claims borrowed partners that the shortlist does not contain", async () => {
    ...
    expect(recommendations.map((r) => r.partnerId)).toEqual([1, 2]);
    // R13 §4.3: the honest four-count framing — full city total, shown count,
    // remainder on request. Borrowed partners not shown are never advertised
    // as "nearby from Dortmund".
    expect(disclosure).toContain("3 in Bochum");
    expect(disclosure).toContain("showing 2");
  ...
  it("discloses that the list is not exhaustive when the working set was larger", async () => {
    ...
    expect(disclosure).toContain("showing 2");
    expect(disclosure).toContain("2 more available on request");
  });
