 ...
 *   1. Convex    — row counts from migrations:verifySetup
...
async function main() {
  // 1. Convex
  try {
    const { counts } = await getAdminConvex().action(adminRefs.verifySetup, {});
    if (counts.activePartners === 0) {
      throw new Error("0 active partners — has `npm run convex:seed` been run?");
    }
    ok(
      "convex",
      `${counts.activePartners} active partners, ${counts.partnerEmbeddings} vectors, ` +
        `${counts.cityCentroids} cities`,
    );
  } catch (e) {
    bad("convex", e);
  }
