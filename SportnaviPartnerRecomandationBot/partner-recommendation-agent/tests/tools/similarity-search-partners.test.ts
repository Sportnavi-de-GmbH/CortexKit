...
describe("similaritySearchPartners", () => {
  ...
  it("does not apply similarityThreshold — returns all RPC hits regardless of similarity", async () => {
    vi.mocked(embedText).mockResolvedValue([0.1, 0.2, 0.3]);
    const supabase = fakeSupabase({ rpc: { match_partners: { data: rows, error: null } } });
  ...
  it("degrades to text-only search when embedText throws, and returns a warning (E25)", async () => {
    ...
    expect(supabase.rpcCalls[0].args).toMatchObject({ query_embedding: null });
  });
  ...
  it("passes only the `city` filter key to match_partners (no unsupported is_active key)", async () => {
    ...
    const supabase = fakeSupabase({ rpc: { match_partners: { data: [], error: null } } });

    await similaritySearchPartners({ city: "Bochum", intent: { text: "x", tags: [] }, k: 5 }, supabase);

    expect(supabase.rpcCalls[0].args).toMatchObject({ filters: { city: "Bochum" } });
  });
...
describe("similaritySearchPartners — tag filter wiring", () => {
  ...
  it("passes intent tags through to match_partners' filters", async () => {
    const supabase = fakeSupabase({ rpc: { match_partners: { data: [row], error: null } } });
    await similaritySearchPartners(
      { city: "Dortmund", intent: { text: "yoga", tags: ["yoga", "pilates"] }, k: 5 },
      supabase,
    );
    const args = supabase.rpcCalls[0].args as { filters: Record<string, unknown> };
  ...
  it("requireTagMatch keeps rows the RPC scored as overlapping", async () => {
    const supabase = fakeSupabase({ rpc: { match_partners: { data: [row], error: null } } });
    const { hits } = await similaritySearchPartners(
      {
        city: "Dortmund",
...
describe("similaritySearchPartners — shared embedding", () => {
  ...
  it("uses a pre-computed embedding instead of embedding again", async () => {
    ...
    const args = supabase.rpcCalls[0].args as { query_embedding: unknown };
    expect(args.query_embedding).toBe(vector);
    expect(warnings).toEqual([]);
  });
  ...
  it("applies a supplied AbortSignal to the match_partners RPC call", async () => {
    vi.mocked(embedText).mockResolvedValue([0.1, 0.2, 0.3]);
    const supabase = fakeSupabase({ rpc: { match_partners: { data: [row], error: null } } });
