...
/**
 * Rows as returned by the `city_centroids()` RPC: already grouped per city,
 ...
 * spelling-variant collapse (E21) is asserted against slugify_tag in the
...
const centroidsOk = (rows: unknown[]) => ({
  rpc: { city_centroids: { data: rows, error: null } },
});
...
describe("findNearbyCities", () => {
  ...
  it("aggregates via the city_centroids RPC, never an unbounded partners scan", async () => {
    ...
    expect(supabase.rpc).toHaveBeenCalledWith("city_centroids");
  ...
  it("applies a supplied AbortSignal to the city_centroids RPC call", async () => {
    const supabase = fakeSupabase(centroidsOk(centroidRows));
  ...
  it("does not call abortSignal on a cache hit — there's no I/O to bound", async () => {
    const supabase = fakeSupabase(centroidsOk(centroidRows));
    const cache = createTtlCache<any>(60);
    const controller = new AbortController();
    await findNearbyCities({ home, limit: 5, maxDistanceKm: null }, { supabase, cache });
    ...
    expect(supabase.rpc).toHaveBeenCalledTimes(1); // second call was a cache hit
  ...
  it("consumes the RPC's collapsed spelling-variant node as one city (E21)", async () => {
    // The collapse itself now happens in SQL (`group by slugify_tag(city)`),
  ...
  it("still excludes the home city when the RPC labels it with a variant spelling (E21)", async () => {
    // normalizeCityKey remains in use for home/alias exclusion, which happens
    // client-side against whatever label the RPC chose.
    const supabase = fakeSupabase(
      centroidsOk([
