...
/** The shape find_partners actually returns (agent/tools/find_partners.ts). */
const SEARCH_RESULT = {
...
describe("failure spans", () => {
  ...
  it("explains the silent-zero-rows trap when Supabase refuses", () => {
    const span = failureSpan({
      ...base,
      kind: "tool",
      subject: "find_partners",
      data: { message: "row-level security policy violation" },
    });
    expect(span.attributes["app.recommended_action"]).toMatch(/SERVICE ROLE/i);
  });
...
describe("hook", () => {
  ...
  it("emits an ERROR span for a failed tool call", async () => {
    ...
    await h["action.result"](
      { data: { result: { toolName: "find_partners", isError: true, output: "supabase down" } } },
      ctx,
    );
    expect(spans).toHaveLength(1);
    expect(spans[0].name).toBe("failure:tool");
    expect(spans[0].error).toContain("supabase down");
  });
