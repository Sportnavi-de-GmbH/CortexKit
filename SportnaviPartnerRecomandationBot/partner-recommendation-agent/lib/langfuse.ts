...
export const PARTNER_KNOWLEDGE_SOURCE =
  "Supabase `partners` table via lib/partners (resolve city -> gap-fill from nearby cities -> rank -> hydrate profiles)";
...
export const TOOL_STORIES: Record<string, ToolStory> = {
  find_partners: {
    ...
    errors: [
      {
        match: /supabase|PGRST|relation .* does not exist|JWT|row-level security/i,
        ...
        recommended_action:
          "Check MEMORY_SUPABASE_URL and MEMORY_SUPABASE_SERVICE_ROLE_KEY. RLS is ON with no policies, so the ANON key silently returns ZERO rows instead of erroring — confirm the SERVICE ROLE key is the one in use.",
      },
      {
        match: /embedding/i,
  ...
  get_partner_details: {
    ...
    errors: [
      {
        match: /supabase|PGRST|not found/i,
...
export const FAILURE_PATTERNS: {
  ...
  {
    match: /supabase|PGRST|row-level security|JWT/i,
    error_type: "upstream_unavailable",
    cause:
      "The Supabase partner directory refused or failed the query. RLS is on with no policies, so the ANON key returns zero rows silently rather than erroring.",
    recommended_action:
      "Confirm MEMORY_SUPABASE_SERVICE_ROLE_KEY (service role, not anon) is set for this environment.",
  },
  {
    match: /embedding/i,
    error_type: "upstream_unavailable",
