...
export function MessageList({ agent, className = "px-10 py-10" }: { agent: Agent; className?: string }) {
  ...
  // A turn can die server-side without ever surfacing as `status === "error"`:
  // e.g. Azure's content filter 400s the request (observed live 2026-08-20 —
  // "studios having pain" tripped the sexual/high filter), eve logs a harness
  // event, the stream ends, and the user gets SILENCE. Detect "the turn is
  // over and no assistant text ever arrived" and show a warm fallback instead
  // of nothing. Never expose the raw upstream error (rule #9 applies to the
  // UI too).
  ...
  const lastHasAssistantText =
    last?.role === "assistant" &&
    last.parts.some(
      (p: EveMessagePart) => p.type === "text" && typeof p.text === "string" && p.text.trim().length > 0,
    );
  const turnDiedSilently =
    !isBusyNow && agent.status !== "error" && messages.length > 0 && !lastHasAssistantText;
    ...
    <div ref={scrollRef} className={`flex-1 overflow-y-auto ${className}`}>
      ...
      <div className="mx-auto flex max-w-3xl flex-col gap-8">
        ...
        {agent.status === "error" && agent.error && (
          UPSTREAM_ERROR_RE.test(agent.error.message) ? (
            <FriendlyTurnError />
          ) : (
        ...
        {turnDiedSilently && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[13px] leading-relaxed text-amber-800">
            Das konnte ich gerade leider nicht beantworten 😅 Formulier deine
            Anfrage bitte einmal etwas anders — dann versuche ich es sofort
            nochmal 💚 / I couldn&apos;t answer that one — please rephrase and
...
/** Upstream/provider failures that eve surfaces as message text. Raw API
 * error prose must never reach the user (rule #9 applies to the UI too) —
 * observed live 2026-08-20: Azure's content filter 400 rendered verbatim. */
const UPSTREAM_ERROR_RE =
  /content management policy|content_filter|context_length_exceeded|Please modify your prompt and retry|ResponsibleAIPolicyViolation|upstreamStatusCode|rate limit/i;

function FriendlyTurnError() {
  return (
    <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[13px] leading-relaxed text-amber-800">
      Das konnte ich gerade leider nicht beantworten 😅 Formulier deine Anfrage
      bitte einmal etwas anders — dann versuche ich es sofort nochmal 💚 / I
      couldn&apos;t answer that one — please rephrase and I&apos;ll try again
      right away.
    </div>
  );
}
...
function PartView({ part, agent }: { part: EveMessagePart; agent: Agent }) {
  switch (part.type) {
    case "text":
      if (UPSTREAM_ERROR_RE.test(part.text)) return <FriendlyTurnError />;
      return (
