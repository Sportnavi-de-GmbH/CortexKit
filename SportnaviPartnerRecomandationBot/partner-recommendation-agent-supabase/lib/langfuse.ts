...
export const PARTNER_AGENT_ROLE =
  "Navio partner-recommendation agent — finds real Sportnavi studios and courses near a city from the Supabase partner directory, and never invents one";
...
export const PARTNER_KNOWLEDGE_MODE =
  "retrieval — Supabase partner directory + embedding similarity (the system prompt holds behaviour rules only, no partner data)";
export const PARTNER_KNOWLEDGE_SOURCE =
  "Supabase `partners` table + `match_partners` RPC via lib/partners (resolve city -> gap-fill from nearby cities -> rank -> hydrate profiles)";
 ...
 *  the tool would pull the whole Supabase/embedding pipeline into the hook
...
export const TOOL_STORIES: Record<string, ToolStory> = {
  find_partners: {
    ...
    errors: [
      {
        match: /supabase|postgrest|PGRST|MEMORY_SUPABASE|Could not find the function|fetch failed/i,
        ...
        recommended_action:
          "Check MEMORY_SUPABASE_URL / MEMORY_SUPABASE_SERVICE_ROLE_KEY (lib/supabase.ts; the SERVICE ROLE key is mandatory — the anon key returns zero rows silently). 'Could not find the function' means an RPC (resolve_city_fuzzy, city_centroids, match_partners, get_partner_profiles) is missing from the database — run `npm run supabase:verify`.",
      },
      {
        match: /embedding/i,
      ...
      },
      {
        ...
        recommended_action:
          "Check Supabase and embedding latency. The deadline bounds the pipeline as a whole; per-stage timeouts are separate.",
      },
    ],
  },
  get_partner_details: {
    ...
    errors: [
      {
        match: /supabase|postgrest|PGRST|not found/i,
        ...
        recommended_action: "Check the Supabase connection and that the partner id still exists.",
      },
    ],
  },
};
...
export const FAILURE_PATTERNS: {
  ...
  },
  {
    ...
    recommended_action:
      "Check Azure health, Supabase/embedding latency, and the 15s find_partners deadline.",
  },
  {
    match: /supabase|postgrest|PGRST|MEMORY_SUPABASE|Could not find the function|fetch failed/i,
    error_type: "upstream_unavailable",
    cause:
      "The Supabase partner directory refused or failed the query — wrong/unset project URL, the anon key instead of the service-role key (RLS returns zero rows silently), or a missing RPC.",
    recommended_action:
      "Confirm MEMORY_SUPABASE_URL / MEMORY_SUPABASE_SERVICE_ROLE_KEY for this environment, then run `npm run supabase:verify` (S1–S6) against it.",
  },
  {
    match: /embedding/i,
    error_type: "upstream_unavailable",
