 ...
 *   1. Supabase  — row counts + the city_centroids() RPC
...
async function main() {
  ...
  try {
    ...
    if (!url || !key) throw new Error("MEMORY_SUPABASE_URL / MEMORY_SUPABASE_SERVICE_ROLE_KEY unset");
    const sb = createClient(url, key, { auth: { persistSession: false } });

    const { count: active, error } = await sb
      .from("partners")
      .select("id", { count: "exact", head: true })
      .eq("is_active", true);
    if (error) throw new Error(`partners count: ${error.message}`);
    if (!active) {
      throw new Error("0 active partners — is this the service-role key? (RLS returns zero rows to anon)");
    }
    const { count: vectors, error: vecErr } = await sb
      .from("partners")
      .select("id", { count: "exact", head: true })
      .eq("is_active", true)
      .not("profile_embedding", "is", null);
    if (vecErr) throw new Error(`embedding count: ${vecErr.message}`);

    const centroids = await getSupabase().cityCentroids();
    ok("supabase", `${active} active partners, ${vectors ?? 0} vectors, ${centroids.length} cities`);
  } catch (e) {
    bad("supabase", e);
  }
