...
describe("recordKnownNames", () => {
  ...
  // R13 §3.2 (plans/R03): known names are CONVERSATION-scoped — a follow-up
  // naming a turn-1 partner must not trip the grounding tripwire. Only a dead
  // session (clearBudget) drops them.
  it("survives resetBudget (conversation-scoped) and is dropped by clearBudget", () => {
    const session = sid();
    recordKnownNames(session, ["Studio A"]);
    resetBudget(session);
    expect(getBudgetSnapshot(session).knownPartnerNames).toEqual(["Studio A"]);
    clearBudget(session);
