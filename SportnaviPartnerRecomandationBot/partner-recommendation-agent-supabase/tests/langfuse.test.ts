...
describe("failure spans", () => {
  ...
  it("explains the missing-RPC trap when Supabase refuses", () => {
    const span = failureSpan({
      ...base,
      kind: "tool",
      subject: "find_partners",
      data: { message: "Could not find the function public.resolve_city_fuzzy(place) in the schema cache" },
    });
    expect(span.attributes["app.recommended_action"]).toMatch(/supabase:verify/i);
  });
