// User-feedback invariants for the PARTNER agent, offline.
//
// lib/feedback.ts is duplicated per service (same reason as lib/langfuse.ts:
// separate deployments, separate credentials, separate Langfuse projects), so
// its tests are duplicated too — otherwise an edit to this copy would ship
// untested while the FAQ copy stayed green.
//
