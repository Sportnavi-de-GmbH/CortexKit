...
import { fakeSupabase, partnerRow } from "./_fakes";
...
describe("getPartnersByCity", () => {
  ...
  it("dedupes by id across overlapping aliases (E8)", async () => {
    const supabase = fakeSupabase({ getPartnersByCity: [rowKoeln, rowKoelnDup, rowKoeln2] });
  ...
  it("passes every alias through so the backend can match all spellings", async () => {
    const supabase = fakeSupabase({ getPartnersByCity: [rowKoeln] });
    await getPartnersByCity({ aliases: ["Köln", "Koeln"] }, supabase);
    expect(supabase.callsTo("getPartnersByCity")[0]!.args).toMatchObject({
      aliases: ["Köln", "Koeln"],
    });
  });
  ...
  it("returns every matched row unranked and untrimmed — no limit (golden rule)", async () => {
    ...
    const supabase = fakeSupabase({ getPartnersByCity: many });
  ...
  it("forwards tagFilter only when provided and non-empty", async () => {
    ...
    expect(supabase.callsTo("getPartnersByCity")[0]!.args).not.toHaveProperty("tagFilter");
    ...
    expect(supabase.callsTo("getPartnersByCity")[1]!.args).not.toHaveProperty("tagFilter");

    await getPartnersByCity({ aliases: ["Köln"], tagFilter: ["klettern"] }, supabase);
    expect(supabase.callsTo("getPartnersByCity")[2]!.args).toMatchObject({
      tagFilter: ["klettern"],
    });
  });
  ...
  it("leaves is_active filtering to the backend by default, and forwards includeInactive (E18)", async () => {
    ...
    await getPartnersByCity({ aliases: ["Köln"] }, supabase);
    expect(supabase.callsTo("getPartnersByCity")[0]!.args).not.toHaveProperty("includeInactive");

    await getPartnersByCity({ aliases: ["Köln"], includeInactive: true }, supabase);
    expect(supabase.callsTo("getPartnersByCity")[1]!.args).toMatchObject({
  ...
  it("still returns partners with no coordinates as home partners (E17)", async () => {
    ...
    const supabase = fakeSupabase({ getPartnersByCity: [noCoords] });
    const rows = await getPartnersByCity({ aliases: ["Köln"] }, supabase);
  ...
  it("applies the given AbortSignal to the backend call when supplied", async () => {
    ...
    const calls = supabase.callsTo("getPartnersByCity");
  ...
  it("passes no signal when none is supplied", async () => {
    const supabase = fakeSupabase({ getPartnersByCity: [rowKoeln] });
    await getPartnersByCity({ aliases: ["Köln"] }, supabase);
    expect(supabase.callsTo("getPartnersByCity")[0]!.signal).toBeUndefined();
  });
});
