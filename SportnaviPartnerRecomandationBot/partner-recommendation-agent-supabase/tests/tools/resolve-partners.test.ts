...
describe("resolvePartners — embeds once for the whole gap-fill fan-out", () => {
  ...
  // R13 §4.1: the embed is HOISTED — home relevance scoring needs it even
  // when gap-fill has no candidate cities. Still exactly once per search.
  it("embeds exactly once even when there are no candidate cities to search", async () => {
