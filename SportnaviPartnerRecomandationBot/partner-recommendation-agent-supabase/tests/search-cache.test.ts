...
describe("searchCacheKey", () => {
  it("normalizes city casing and whitespace to one entry", () => {
    const base = { tags: ["yoga"],  intentText: "Yoga" };
  ...
  it("is order-insensitive for tags", () => {
    const base = { cityMention: "Dortmund",  intentText: "Yoga" };
  ...
  it("separates different tag sets — they change which partners come back", () => {
    const base = { cityMention: "Dortmund",  intentText: "Sport" };
  ...
  // R13 §7: `finalRecommendations` deliberately left OUT of the key — the
  // cached value is the resolved+scored+hydrated SEARCH CORE; shortlist
  // sizing is presentation-level. "Show me more" must be a cache HIT.
  it("ignores the result count — sizing is presentation-level (R13 §7)", () => {
    const a = searchCacheKey({ cityMention: "Dortmund", tags: ["yoga"], intentText: "Yoga" });
    const b = searchCacheKey({ cityMention: "Dortmund", tags: ["yoga"], intentText: "Yoga" });
    expect(a).toBe(b);
  });
  ...
  it("separates different intents even when city and tags are identical", () => {
    const base = { cityMention: "Bochum", tags: [] };
  ...
  it("normalizes intent casing and whitespace so trivial rewording still hits", () => {
    const base = { cityMention: "Bochum", tags: ["yoga"] };
