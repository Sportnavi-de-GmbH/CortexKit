/**
 * load-env.ts — loads .env.local from the project root into process.env for
 * scripts run outside the eve runtime (tsx scripts, tests). `import
 * "dotenv/config"` alone only reads `.env`, which this project intentionally
 * does not use — secrets live in `.env.local`.
 *
