...
//
// This check posts one red (breach) and one green (recovery) test card to the real "Navio Alerts"
// Teams channel — that is intentional, it is how delivery is proven end to end.
//
...
const skip = (label: string, detail = "") => { console.log(`SKIP  ${label.padEnd(50)} ${detail}`); };
...
async function evaluate(slot: "test" | "manual" = "manual") {
  ...
  const body = (await res.json()) as {
    slot: string;
    transitions: { kind: string; rule_key: string; agent: string; delivery?: { teams: string } }[];
    observations: { rule_key: string; agent: string; status: string; samples: number }[];
    digest: { sent: boolean };
  };
  if (body.slot) seenSlots.add(body.slot);
...
async function cleanup() {
  if (seenSlots.size > 0) await db!.from("alert_events").delete().in("run_slot", [...seenSlots]);
...
(async () => {
  try {
    ...
    const baseline = await evaluate("manual");
    const baselineObs = baseline.body.observations.find((o) => o.rule_key === "failure_rate" && o.agent === "faq");

    if (baselineObs?.status === "breached") {
      skip("breach/recovery checks (failure_rate·faq already breached by real traffic)");
    } else {
      // 1. seed a breach: enough failed production traces to push failure_rate well past 10%
      const n = Math.max(30, Math.ceil(2 * (baselineObs?.samples ?? 0)));
      await seed(n, "failed");
