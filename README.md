# CortexKit — Navio (Sportnavi Support Chatbot)

This repository contains **Navio**, the AI support chatbot for Sportnavi, plus
its evaluation harness, prompt-optimization workspace, and full documentation.

> New here? Start with **[`docs/`](docs/)** for the complete guided documentation,
> or **[`CLAUDE.md`](CLAUDE.md)** for a terse architecture map.

## Repository structure

```
CortexKit/
├── kb-agent-langsmith-starter/     # THE project: the Navio agent + eval harness (Next.js + eve app)
│   ├── agent/                      #   the agent: instructions.md (system prompt + KB), tools, hooks
│   ├── app/  components/  public/  #   Next.js dev chat console
│   ├── lib/  scripts/  src/        #   LangSmith integration, eval runners, helpers
│   ├── tests/                      #   unit / smoke tests
│   ├── evals/datasets/             #   evaluation datasets (60-sample benchmark + 10-sample dev subset)
│   ├── system_prompt_optimization/ #   prompt candidates (baseline, v1..v6) + analysis reports
│   ├── knowledge_base_improvement/ #   KB-gap backlog + team Excel/CSV to enrich the knowledge base
│   └── (package.json, tsconfig, next.config, …)  # standard app config at the app root
│
├── docs/                           # Complete project documentation (01..11 + index)
│   └── reference/                  #   long-form guides: EVE_LANGSMITH_TRACING_GUIDE, PROJECT_DOCUMENTATION
├── CLAUDE.md                       # Terse architecture reference
├── project-prompts/                # Task prompts used to drive this repo's work
├── embed-test/                     # Manual widget-embed test page (plain HTML, dev only)
├── archive/                        # Deprecated / duplicate files, kept for history (never deleted)
└── .mcp.json (gitignored)          # local MCP config with secrets — never committed
```
...
# or:  npm run dev:ui          # Next.js chat console
```

Evaluate a prompt change:
```bash
npm run eval:dev               # fast: 10 curated samples (~6 min)
npm run eval:run               # full: 60 samples, 7 judges (~35 min)
```

Full setup, environment variables, testing, and troubleshooting:
...
| I want to… | Go to |
|---|---|
| Understand why this exists & how it works | [`docs/`](docs/) (start at the README) |
| Change the chatbot's behavior or facts | `kb-agent-langsmith-starter/agent/instructions.md` |
| Compare prompt versions | `kb-agent-langsmith-starter/system_prompt_optimization/` |
| Improve the knowledge base (team) | `kb-agent-langsmith-starter/knowledge_base_improvement/` |
| Run/extend evaluations | `kb-agent-langsmith-starter/scripts/` + `evals/datasets/` |

## Conventions

- **The prompt is the product.** Behavior/facts change by editing
  `agent/instructions.md`; no code change needed. Always evaluate a change
  (`eval:dev`) before promoting it.
- **Secrets never get committed.** `.env.local` and `.mcp.json` are gitignored.
  Only placeholders live in `.env.example`.
- **The core folder layout is framework-locked** (Next.js + eve). Don't relocate
  `agent/`, `app/`, `lib/`, `scripts/`, `src/`, `tests/`, `evals/`, or the root
  config files — paths are hardcoded and expected by the framework/runners.
